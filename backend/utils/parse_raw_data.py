...
def load_data():
    ...
    with open(title_crew_path, newline="", encoding="utf-8") as file:
        reader = csv.reader(file, delimiter="\t")
        title_crew_fields = next(reader)
        print(title_crew_fields)
        for row in reader:
            zipped_data = dict(zip(title_crew_fields, row))
            title_crew_data[zipped_data["tconst"]] = zipped_data

def collect_top_rated_movies():
    global title_ratings_data
